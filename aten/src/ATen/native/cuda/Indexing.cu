...
#ifdef USE_ROCM
  // Number of values processed by each thread (grain size)
  union x2lng { int64_t l[2]; long2 l2; } tmpl2; // avoid serilized loads by fetching 2 at a time.
  for (int64_t z = blockIdx.z; z < outer_dim; z += gridDim.z)
  for (int64_t _y = 0; _y < 4; _y++) {
    ...
    if (_y > 0)         { 
			  ...
			  else tmpl2.l[1] = sorted_indices[idx]; 
                        }
    else if (idx != 0 )   tmpl2.l2   = *((long2*)(&sorted_indices[idx - 1]));
    else                {
    ...
    if (oneDup && accumulate && (crnt_sorted_idx != tmpl2.l[0])) { // skip duplicate walk and wave reduce
