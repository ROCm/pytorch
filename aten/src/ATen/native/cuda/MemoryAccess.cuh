...
__device__ aligned_vector<scalar_t, vec_size> load_vector(const scalar_t *base_ptr, uint32_t offset) {
  ...
  // Do NOT extend to gfx1250/gfx1250-strict (GFX12.5, Wave32) without first
  // validating intrinsic codegen and re-benchmarking -- the register layout
  // and wave width differ. Those archs fall through to the generic path below,
  // which is correctness-safe.
