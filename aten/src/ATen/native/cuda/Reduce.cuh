...
ReduceConfig setReduceConfig(const TensorIterator& iter){
  ...
  if (config.input_mult[1] != 0 && config.values_per_thread() >= max_values_per_thread && grid <= target_grid_size) {
    ...
    // MPs. For smaller sizes than half the number of MPs use the original value unless the
    // value is less than 16 blocks in which case it is more profitable to use just 1 block.
