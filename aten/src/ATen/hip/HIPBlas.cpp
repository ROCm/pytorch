...
class CuBlasSMCarveoutGuard {
 public:
  explicit CuBlasSMCarveoutGuard(hipblasHandle_t handle) : handle_(handle) {
...
#if !defined(USE_ROCM) && defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 13030
...
static inline bool bgemm_internal_cublaslt(CUDABLAS_BGEMM_ARGTYPES_AND_C_DTYPE(Dtype, C_Dtype)) {
  ...
  if constexpr (std::is_same_v<Dtype, float>) {
    if (detail::cublasOpFromChar(transa) == HIPBLAS_OP_T && detail::cublasOpFromChar(transb) == HIPBLAS_OP_T) {
        return false;
    }
  }
#endif
  const auto type_info = detail::getCublasLtTypeInfo<Dtype, C_Dtype>();
  const hipDataType abType = type_info.ab_type;
  const hipDataType cType = type_info.c_type;
  const hipblasComputeType_t computeType = type_info.compute_type;
  const hipDataType scaleType = type_info.scale_type;
...
#ifndef USE_ROCM
    if (computeType == HIPBLAS_COMPUTE_16F) {
      halpha = alpha;
      hbeta = beta;
      alpha_ptr = &halpha;
      beta_ptr = &hbeta;
  ...
  hipblasOperation_t opa = detail::cublasOpFromChar(transa);
  hipblasOperation_t opb = detail::cublasOpFromChar(transb);
  ...
  if (lie_to_cublaslt) {
     ...
     CuBlasLtMatrixLayout FakeBdesc(abType, k, 2, fake_ldb, opb == HIPBLAS_OP_T);
     ...
     if (num_batches > 1) {
       ...
       FakeCdesc.setAttribute(
           HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT, num_batches_as_int);
       FakeBdesc.setAttribute(
           HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, strideb);
       FakeCdesc.setAttribute(
           HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, stridec);
     }
...
#if !defined(USE_ROCM) && defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 12090
  if (useBF16x9()) {
...
inline void bgemm_internal_cublas_half_helper(CUDABLAS_BGEMM_ARGTYPES_AND_C_DTYPE(at::Half, C_Dtype)) {
  ...
  void * beta_ptr = &fbeta;
#ifdef USE_ROCM
  auto raw_handle = static_cast<hipblasHandle_t>(handle);
  int flag = 0;
...
inline void gemm_internal_cublas_half_helper(CUDABLAS_GEMM_ARGTYPES_AND_C_DTYPE(at::Half, C_Dtype)) {
  ...
  if (prop->major >= 5) {
    ...
    if (fp16_reduction !=
        at::CuBLASReductionOption::AllowReducedPrecisionWithSplitK) {
      cublas_flags = static_cast<hipblasMath_t>(
          cublas_flags | HIPBLAS_MATH_DISALLOW_REDUCED_PRECISION_REDUCTION);
    }
...
bool gemm_and_bias(
  ...
  // Cdesc rather than paying for a second hipblasLtMatrixLayoutCreate on what is
...
void scaled_gemm(
  ...
  CuBlasLtMatmulDescriptor computeDesc(computeType, scaleType);
  computeDesc.setAttribute(HIPBLASLT_MATMUL_DESC_TRANSA, detail::cublasOpFromChar(transa));
  computeDesc.setAttribute(HIPBLASLT_MATMUL_DESC_TRANSB, detail::cublasOpFromChar(transb));
...
void grouped_gemm(
  ...
  TORCH_CHECK(prop->major >= 9 && prop->major < 12, "grouped hipblasLtMatmul requires SM 9.0-11.0");

  const auto computeType = HIPBLAS_COMPUTE_32F;
  const auto scaleType = HIP_R_32F;
  const auto pointer_mode = HIPBLASLT_POINTER_MODE_DEVICE;
  ...
  CuBlasLtGroupedMatrixLayout Adesc(ScalarTypeToCudaDataType(input_dtype), batchCount, mArrayDev, kArrayDev, ldaArrayDev, opa != HIPBLAS_OP_N, use_int64_dims);
  CuBlasLtGroupedMatrixLayout Bdesc(ScalarTypeToCudaDataType(input_dtype), batchCount, kArrayDev, nArrayDev, ldbArrayDev, opb != HIPBLAS_OP_N, use_int64_dims);
  ...
  TORCH_CHECK(
      ...
      "CUDA error: ",
      at::cuda::blas::_cublasGetErrorEnum(cublasStatus),
      " when calling grouped hipblasLtMatmul");
  return;
#else
  TORCH_CHECK(false, "grouped hipblasLtMatmul requires CUDA >= 13.3 and is not supported on ROCm. Current build does not meet these requirements.");
#endif // !defined(USE_ROCM) && defined(TORCH_HIP_VERSION) && TORCH_HIP_VERSION >= 13030
}
...
void gemv<c10::complex<double>>(CUDABLAS_GEMV_ARGTYPES(c10::complex<double>)) {
  auto handle = at::cuda::getCurrentCUDABlasHandleWithWorkspace();
  hipblasOperation_t op = detail::cublasOpFromChar(trans);
...
void gemv<at::Half>(CUDABLAS_GEMV_ARGTYPES(at::Half)) {
  ...
  bool trans_bool = (detail::cublasOpFromChar(trans) != HIPBLAS_OP_N);
  if (trans_bool) {
    std::swap(m, n);
  }
