...
class CuBlasLtMatrixLayout : public CuBlasLtDescriptor<
 ...
 public:
  CuBlasLtMatrixLayout(
    ...
    TORCH_CUDABLAS_CHECK(hipblasLtMatrixLayoutCreate(
  ...
  void setAttribute(hipblasLtMatrixLayoutAttribute_t attr, const T value) {
    TORCH_CUDABLAS_CHECK(::hipblasLtMatrixLayoutSetAttribute(
...
inline hipblasOperation_t cublasOpFromChar(char op) {
  switch (op) {
    case 'n':
    case 'N':
      return HIPBLAS_OP_N;
  ...
  TORCH_CHECK(
      ...
      op,
      "`");
  return HIPBLAS_OP_N; // unreachable - TORCH_CHECK(false) throws
}
...
CublasLtTypeInfo<T, C_Dtype> getCublasLtTypeInfo() {
  ...
  if constexpr (std::is_same_v<T, double>) {
    info.ab_type = HIP_R_64F;
    info.c_type = HIP_R_64F;
    info.compute_type = HIPBLAS_COMPUTE_64F;
    info.scale_type = HIP_R_64F;
  } else if constexpr (std::is_same_v<T, float>) {
    const auto fp32_precision = at::globalContext().float32Precision(
        at::Float32Backend::CUDA, at::Float32Op::MATMUL);
    if (fp32_precision == at::Float32Precision::TF32) {
      info.compute_type = HIPBLAS_COMPUTE_32F_FAST_TF32;
    }
    ...
    info.ab_type = HIP_C_64F;
    info.c_type = HIP_C_64F;
    info.compute_type = HIPBLAS_COMPUTE_64F;
    info.scale_type = HIP_C_64F;
  } else if constexpr (std::is_same_v<T, c10::complex<float>>) {
    info.ab_type = HIP_C_32F;
    info.c_type = HIP_C_32F;
    info.scale_type = HIP_C_32F;
  } else if constexpr (std::is_same_v<T, at::Half>) {
#ifndef USE_ROCM
    auto* prop = at::cuda::getCurrentDeviceProperties();
    if (prop->major >= 7 && at::globalContext().allowFP16AccumulationCuBLAS()) {
      info.compute_type = HIPBLAS_COMPUTE_16F;
      info.scale_type = HIP_R_16F;
    }
#endif
    info.ab_type = HIP_R_16F;
    info.c_type = std::is_same_v<C_Dtype, float> ? HIP_R_32F : HIP_R_16F;
  } else if constexpr (std::is_same_v<T, at::BFloat16>) {
    info.ab_type = HIP_R_16BF;
    info.c_type = std::is_same_v<C_Dtype, float> ? HIP_R_32F : HIP_R_16BF;
  } else {
    static_assert(
        false && sizeof(T),
...
inline hipblasLtEpilogue_t cublasLtEpilogue(
