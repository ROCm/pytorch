...
struct TORCH_CUDA_CPP_API CUDAGraph {
  ...
  // operate on the captured hipGraph_t before finalization.
  void capture_end_pre();
  void capture_end_post();
  void instantiate();
  // True once the hipGraphExec_t has been instantiated (by capture_end when
 ...
 protected:
  ...
  struct OwnedCUDAStream {
    ...
    explicit OwnedCUDAStream(hipStream_t s) : stream(s) {}
    ~OwnedCUDAStream() {
      if (stream)
        C10_CUDA_CHECK_WARN(hipStreamDestroy(stream));
    }
