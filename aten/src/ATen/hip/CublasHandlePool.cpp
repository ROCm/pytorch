...
void setWorkspaceForHandle(hipblasHandle_t handle, c10::cuda::CUDAStream stream) {
  ...
  // Insert with lock, replacing any undersized entry
  {
    ...
    TORCH_CUDABLAS_CHECK(
        hipblasSetWorkspace(handle, workspace_it->second.first.get(), workspace_size));
  }
}
...
static void setupCUDABlasHandle(
  ...
  switch (workspace_mode) {
    ...
    case WorkspaceMode::Default:
      // hipblasSetStream above resets the handle to cuBLAS's default workspace.
      break;
  }
  ...
  // To enable TF32, set the math mode of the handle to HIPBLAS_TF32_TENSOR_OP_MATH.
  if (!NoTF32Guard::should_disable_fp32_reduced_precision() &&
    ...
    TORCH_CUDABLAS_CHECK(hipblasSetMathMode(handle, HIPBLAS_TF32_TENSOR_OP_MATH));
  } else {
...
static hipblasHandle_t getCurrentCUDABlasHandleImpl(
    void* workspace,
    size_t workspace_size,
    WorkspaceMode workspace_mode,
    bool setup) {
  ...
  thread_local std::unique_ptr<CuBlasPoolType::PoolWindow> myPoolWindow(
      pool->newPoolWindow());
  hipblasHandle_t handle = myPoolWindow->reserve(device);
...
CUDABlasHandleWithWorkspace::~CUDABlasHandleWithWorkspace() {
  ...
  const hipblasStatus_t status = hipblasSetStream(handle_, stream_);
  if (C10_UNLIKELY(status != HIPBLAS_STATUS_SUCCESS)) {
