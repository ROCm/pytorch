...
#include <ATen/hip/detail/CublasLtUtils.h>
...
class CublasltGemmProblemBase {
 public:
  ...
  hipDataType a_type() const {
    return a_type_;
  }

  hipDataType b_type() const {
    return b_type_;
  }

  hipDataType c_type() const {
    return c_type_;
  }

  hipDataType d_type() const {
    return d_type_;
  }

  hipblasLtMatmulDesc_t compute_desc() const {
    return compute_desc_.descriptor();
  }

  hipblasLtMatrixLayout_t adesc() const {
    return adesc_->descriptor();
  }

  hipblasLtMatrixLayout_t bdesc() const {
    return bdesc_->descriptor();
  }

  hipblasLtMatrixLayout_t cdesc() const {
    return cdesc_->descriptor();
  }

  // Default: D aliases C. Variants with a distinct D layout hide this.
  hipblasLtMatrixLayout_t ddesc() const {
    return cdesc();
  }
  ...
  hipblasLtMatrixLayout_t heuristic_bdesc() const {
    return bdesc();
  }

  hipblasLtMatrixLayout_t heuristic_cdesc() const {
    return cdesc();
  }

  hipblasLtMatmulPreference_t preference() const {
...
class CublasltStandardGemmProblem : public CublasltGemmProblemBase {
 public:
  CublasltStandardGemmProblem(
    ...
    if constexpr (std::is_same_v<T, at::Half>) {
      if (compute_type_ == HIPBLAS_COMPUTE_16F) {
        halpha_ = alpha_;
        hbeta_ = beta_;
        alpha_ptr_ = &halpha_;
        beta_ptr_ = &hbeta_;
    ...
    compute_desc_.setAttribute(HIPBLASLT_MATMUL_DESC_TRANSA, opa_);
    compute_desc_.setAttribute(HIPBLASLT_MATMUL_DESC_TRANSB, opb_);
    if (set_epilogue_attribute_ || bias_ != nullptr ||
      ...
      compute_desc_.setAttribute(HIPBLASLT_MATMUL_DESC_EPILOGUE, epilogue_);
    }
    if (bias_ != nullptr) {
      compute_desc_.setAttribute(HIPBLASLT_MATMUL_DESC_BIAS_POINTER, bias_);
    }
    ...
    adesc_ = std::make_unique<at::cuda::blas::detail::CuBlasLtMatrixLayout>(
        a_type_, m_, k_, lda_, opa_ != HIPBLAS_OP_N);
    bdesc_ = std::make_unique<at::cuda::blas::detail::CuBlasLtMatrixLayout>(
        b_type_, k_, n_, ldb_, opb_ != HIPBLAS_OP_N);
    cdesc_ = std::make_unique<at::cuda::blas::detail::CuBlasLtMatrixLayout>(
        c_type_, m_, n_, ldc_);

    if (batch_count_ > 1) {
      int batch_as_int = static_cast<int>(batch_count_);
      adesc_->setAttribute(HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT, batch_as_int);
      bdesc_->setAttribute(HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT, batch_as_int);
      cdesc_->setAttribute(HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT, batch_as_int);
      adesc_->setAttribute(
          HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, stride_a_);
      bdesc_->setAttribute(
          HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, stride_b_);
      cdesc_->setAttribute(
          HIPBLASLT_MATRIX_LAYOUT_STRIDED_BATCH_OFFSET, stride_c_);
    }
    ...
    preference_.setAttribute(
        HIPBLASLT_MATMUL_PREF_MAX_WORKSPACE_BYTES, workspace_.size);
    ...
    if (lie_to_cublaslt_) {
      ...
      fake_bdesc_ =
          std::make_unique<at::cuda::blas::detail::CuBlasLtMatrixLayout>(
              b_type_, k_, 2, fake_ldb, opb_ == HIPBLAS_OP_T);
      fake_cdesc_ =
          std::make_unique<at::cuda::blas::detail::CuBlasLtMatrixLayout>(
      ...
      if (batch_count_ > 1) {
        ...
        fake_bdesc_->setAttribute(
            HIPBLASLT_MATRIX_LAYOUT_BATCH_COUNT, batch_as_int);
  ...
  hipblasLtMatrixLayout_t heuristic_ddesc() const {
 ...
 private:
  ...
  hipblasOperation_t opa_;
  hipblasOperation_t opb_;
...
class CublasltScaledGemmProblem : public CublasltGemmProblemBase {
 public:
  explicit CublasltScaledGemmProblem(const ScaledGemmParams<CT>* params)
    ...
    compute_desc_.setAttribute(
        HIPBLASLT_MATMUL_DESC_A_SCALE_POINTER, params_->a_scale_ptr);
    compute_desc_.setAttribute(
        HIPBLASLT_MATMUL_DESC_B_SCALE_POINTER, params_->b_scale_ptr);
    if (params_->c_scale_ptr != nullptr) {
      compute_desc_.setAttribute(
          HIPBLASLT_MATMUL_DESC_D_SCALE_POINTER, params_->c_scale_ptr);
    }
    ...
    if (params_->bias_ptr != nullptr) {
      compute_desc_.setAttribute(
          HIPBLASLT_MATMUL_DESC_BIAS_POINTER, params_->bias_ptr);
      compute_desc_.setAttribute(
          HIPBLASLT_MATMUL_DESC_EPILOGUE, HIPBLASLT_EPILOGUE_BIAS);
      compute_desc_.setAttribute(
          HIPBLASLT_MATMUL_DESC_BIAS_DATA_TYPE, c_type_);
    }
    ...
    int b_scale_mode = at::cuda::blas::detail::cublasLtMatmulScaleMode(
        ...
        params_->use_fast_accum);
    compute_desc_.setAttribute(
        HIPBLASLT_MATMUL_DESC_A_SCALE_MODE, a_scale_mode);
    compute_desc_.setAttribute(
        HIPBLASLT_MATMUL_DESC_B_SCALE_MODE, b_scale_mode);
#endif
...
bool CublasltGetAlgoConfigAttribute(
  ...
  return cublasLtMatmulAlgoConfigGetAttribute(
             algo, attr, value, sizeof(T), &written) == HIPBLAS_STATUS_SUCCESS;
}
...
bool CublasltSetAlgoConfigAttribute(
  ...
  return cublasLtMatmulAlgoConfigSetAttribute(
             algo, attr, &value, sizeof(value)) == HIPBLAS_STATUS_SUCCESS;
}
...
bool CublasltValidateAlgo(
    const ProblemT& problem,
    hipblasLtMatmulAlgo_t* algo) {
  hipblasLtMatmulHeuristicResult_t check_result = {};
...
class CublasltMatmulOp : public Callable<ParamsT> {
 public:
  ...
  TuningStatus Call(const ParamsT* params) override {
    ...
    return status == HIPBLAS_STATUS_SUCCESS ? OK : FAIL;
  }

 private:
  template <typename ProblemT>
  bool InitializeAlgo(const ProblemT& problem, hipblasLtMatmulAlgo_t* algo) {
...
GetCublasltHeuristicCandidates(const ProblemT& problem) {
  ...
  auto status = hipblasLtMatmulAlgoGetHeuristic(
      ...
      &returned_result);
  if (status != HIPBLAS_STATUS_SUCCESS || returned_result == 0) {
    return ret;
  }

  for (int i = 0; i < returned_result; ++i) {
    if (heuristic_results[i].state != HIPBLAS_STATUS_SUCCESS) {
      continue;
    }
