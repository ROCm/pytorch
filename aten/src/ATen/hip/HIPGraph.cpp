...
// capture_end is split so callers can run work on the captured hipGraph_t
...
void CUDAGraph::capture_end_pre() {
  ...
  // captures_underway is cleaned up even if hipStreamEndCapture failed
...
hipGraph_t CUDAGraph::raw_cuda_graph() {
  TORCH_CHECK(has_graph_,
      "No hipGraph_t is available: either capture_end() has not been called, "
      "or the underlying hipGraph_t was destroyed (keep_graph=false, and "
...
void CUDAGraph::begin_capture_to_conditional_node(
  ...
  AT_CUDA_CHECK(cudaGraphAddNode_v2(
      ...
      &params));
#endif
  hipGraph_t conditional_node_child_graph = params.conditional.phGraph_out[0];
...
getCurrentCUDAStream(), &cond_node, nullptr, 1, hipStreamSetCaptureDependencies));
  ...
  hipStream_t raw_child_stream{};
  AT_CUDA_CHECK(hipStreamCreateWithFlags(
      &raw_child_stream, hipStreamNonBlocking));
