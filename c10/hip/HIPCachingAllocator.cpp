...
#include <c10/hip/impl/HIPGraphMemory.h>
...
struct ExpandableSegment {
...
#else
    hipError_t reserve_err = DriverAPI::get()->cuMemAddressReserve_(
        ...
        if (status == hipErrorMemoryAllocation) {
          {
            ...
            (void)hipMemGetInfo(&device_free, &device_total);
            LOG(WARNING)
  ...
  static std::unique_ptr<ExpandableSegment> fromShared(
    ...
    // also support fabric access; otherwise hipMemImportFromShareableHandle
...
#define SYS_pidfd_getfd 438
 ...
 private:
  ...
  void setAccess(c10::DeviceIndex device, size_t begin, size_t end) {
    ...
    std::array<hipMemAccessDesc, num_desc> desc{};
  ...
  struct Handle {
    ...
    // False for handles imported via fromShared but not yet hipMemMap'd, so
    // unmapHandles can skip hipMemUnmap on ranges that were never mapped.
    bool mapped = false;
  };
  struct ShareHeader {
...
static std::string reportProcessMemoryInfo(const hipDeviceProp_t& prop) {
  ...
  static bool nvml_init [[maybe_unused]] = []() {
    return AMDSMI_STATUS_SUCCESS == DriverAPI::get()->nvmlInit_v2_();
  }();
  if (!nvml_init) {
    TORCH_WARN_ONCE(
  ...
  amdsmi_processor_handle nvml_device = nullptr;
...
class DeviceCachingAllocator {
 ...
 public:
  ...
  Block* mallocWithAddress(size_t orig_size, hipStream_t stream, void* addr) {
    const auto requested_addr = reinterpret_cast<uintptr_t>(addr);
    TORCH_CHECK(
  ...
  CaptureInfo stream_get_capture_info(hipStream_t stream) const {
    CaptureInfo info{};
  ...
  // Called by CUDAGraph after hipStreamBeginCapture succeeds. Tracks real
  ...
  // a real capture (where hipEventQuery / hipStreamSynchronize are illegal)
  ...
  // Called by CUDAGraph after hipStreamEndCapture.
  void markCaptureEnd() {
    std::lock_guard<std::recursive_mutex> lock(mutex);
 ...
 private:
  ...
  //   2. Per-stream syscall: hipStreamGetCaptureInfo on the current stream.
  ...
  bool is_capture_context() {
    ...
    hipStream_t stream = cuda::getCurrentCUDAStream(device_id).stream();
...
ska::flat_hash_map<hipStream_t, std::string>&
expandableSegmentReserveClassMap() {
  static ska::flat_hash_map<hipStream_t, std::string> m;
  return m;
}
} // namespace
...
std::string getExpandableSegmentReserveClassForStream(hipStream_t stream) {
  std::lock_guard<std::mutex> lock(expandableSegmentReserveClassMutex());
