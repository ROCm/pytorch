...
amdsmi_processor_handle get_nvml_device(c10::DeviceIndex dev) {
  static bool nvml_init [[maybe_unused]] = []() {
    TORCH_INTERNAL_ASSERT(AMDSMI_STATUS_SUCCESS == DriverAPI::get()->nvmlInit_v2_());
    return true;
  }();
...
bool get_fabric_access(c10::DeviceIndex dev) {
  ...
  if (nvml_device != nullptr) {
    ...
    if (nvml_fabric_result != AMDSMI_STATUS_SUCCESS) {
      TORCH_WARN_ONCE(
