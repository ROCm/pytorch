...
void mallocAsync(
  ...
  if (pytorch_used_bytes[device] + size > pytorch_memory_limits[device]) {
    ...
    if (err == hipErrorMemoryAllocation) {
      ...
      (void)hipGetLastError(); // clear the OOM before retrying
      hipMemPool_t mempool = nullptr;
      if (hipDeviceGetDefaultMemPool(&mempool, device) == hipSuccess) {
        // Sync first: hipMemPoolTrimTo only releases backing whose frees have
        ...
        if (sync_err == hipSuccess) {
          (void)hipMemPoolTrimTo(mempool, 0);
          err = hipMallocAsync(devPtr, size, stream);
          if (err == hipSuccess) {
            LOG(WARNING)
                << "[hipMallocAsync] recovered from an allocation failure by "
...
struct CudaMallocAsyncAllocator : public CUDAAllocator {
  ...
  DeviceStats getDeviceStats(c10::DeviceIndex device) override {
    ...
    if (devs_initialized_flags[device]) {
      ...
      // workload. Tolerate hipErrorNotSupported on drivers without graph-mem
      // attributes (treat as 0).
      auto get_graph_attr =
          [device](hipGraphMemAttributeType attr) -> uint64_t {
        uint64_t value = 0;
        hipError_t err = hipDeviceGetGraphMemAttribute(device, attr, &value);
      ...
      uint64_t graph_reserved_current =
          get_graph_attr(hipGraphMemAttrReservedMemCurrent);
      uint64_t graph_reserved_peak =
          get_graph_attr(hipGraphMemAttrReservedMemHigh);
      uint64_t graph_used_current =
          get_graph_attr(hipGraphMemAttrUsedMemCurrent);
      uint64_t graph_used_peak = get_graph_attr(hipGraphMemAttrUsedMemHigh);
  ...
  void resetPeakStats(c10::DeviceIndex device) override {
    ...
    auto reset_graph_high = [device,
                             &graph_zero](hipGraphMemAttributeType attr) {
      hipError_t err =
          hipDeviceSetGraphMemAttribute(device, attr, &graph_zero);
      if (err == hipErrorNotSupported) {
        (void)hipGetLastError(); // clear the sticky error
        return;
      }
    ...
    reset_graph_high(hipGraphMemAttrReservedMemHigh);
    reset_graph_high(hipGraphMemAttrUsedMemHigh);
  }
