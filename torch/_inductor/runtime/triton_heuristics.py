        ...
        if disable_pointwise_autotuning(inductor_meta) and not (
            ...
            if torch.version.hip:
                ...
                configs += [
                    ...
                    # Optimal configs from greedy set cover analysis (5% tolerance, 13 configs)
                    triton_config_with_settings(size_hints, 64, num_warps=1, num_stages=1),
                    ...
                    triton_config_with_settings(size_hints, 256, num_warps=2, num_stages=1),
                    triton_config_with_settings(size_hints, 1024, num_warps=1, num_stages=1),
                    triton_config_with_settings(size_hints, 1024, num_warps=2, num_stages=1),
                    triton_config_with_settings(size_hints, 1024, num_warps=4, num_stages=1),
                    triton_config_with_settings(size_hints, 2048, num_warps=2, num_stages=1),
                    triton_config_with_settings(size_hints, 2048, num_warps=4, num_stages=1),
                    triton_config_with_settings(size_hints, 4096, num_warps=1, num_stages=1),
                    triton_config_with_settings(size_hints, 4096, num_warps=4, num_stages=1),
                    triton_config_with_settings(size_hints, 8192, num_warps=2, num_stages=1),
                    triton_config_with_settings(size_hints, 8192, num_warps=4, num_stages=1),
                ]
    if len(size_hints) == 2:
